...
if (!$deviceName && !$newStatus) {
    ...
    echo "Missing device name and new port status";
    $logger->error("Port status update failed: Missing both device name and port status.");
    exit;
}
...
if (!$deviceName) {
    ...
    $logger->error("Port status update failed: Missing device name.");
    exit;
}

if (!$newStatus) {
    http_response_code(400);
    echo "Missing new port status";
    $logger->error("Port status update failed: Missing new port status for device '$deviceName'.");
    exit;
}
...
if ($response === false) {
    ...
    echo "Failed to reach device";
    $logger->error("Port status update failed: Unable to contact device '$deviceName' at IP $deviceIp.");
    exit;
}
curl_close($ch);
...
if ($response !== $expected) {
    echo "Device did not confirm port status change";
    $logger->warning("Port status update mismatch: Expected '$expected', but got '$response' from device '$deviceName'.");
    exit;
}
...
echo "Port status for '$deviceName' updated to '$newStatus' successfully.";
$logger->info("Port status for device '$deviceName' updated to '$newStatus' successfully.");
?>
